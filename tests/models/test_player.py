...
def test_player_initialization_defaults():
    player = Player(playerId=1, gold=50)
...
def test_take_damage_reduces_hp_not_below_zero():
    player = Player(playerId=1, gold=0, hp=20)

    player.takeDamage(5)
    assert player.core["hp"] == 19
...
def test_gain_xp_no_level_up():
    player = Player(playerId=1, gold=0)
...
def test_to_dict_returns_correct_structure():
    player = Player(playerId=1, gold=10, hp=80, level=3, xp=40)
    ...
    assert result["hp"] == 80
    assert result["maxhp"] == 100
    assert result["level"] == 3
    assert result["xp"] == 40
    assert result["attack"] == 10
    assert result["defense"] == 5
    assert result["critchance"] == 0.05
    assert result["critmultiplier"] == 1.5
